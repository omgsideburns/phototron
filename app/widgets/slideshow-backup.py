...
class SlideshowWidget(QWidget):
   ...
   # def resizeEvent(self, event):
        ...
        # Target ratio: 2:3 (width:height)
        target_ratio = 3 / 2
        
        label_height = container_height
        label_width = container_height / target_ratio
        ...
        for label in (self.label1, self.label2):
            ...
            # Re-apply pixmap if it exists
            if label.pixmap():
                scaled = label.pixmap().scaled(label.size(), Qt.KeepAspectRatio, Qt.SmoothTransformation)
    ...
    def resizeEvent(self, event):
        ...
        # Fit within parent, maintaining 2:3
        if container_width / container_height > target_ratio:
            # Too wide — use height to determine width
            label_height = container_height
            label_width = int(label_height * target_ratio)
        else:
            # Too tall — use width to determine height
            label_width = container_width
            label_height = int(label_width / target_ratio)
